...
async function createOrders () {
  ...
  const _adminEmail = await security.hashPassword(adminEmail)
  const demo = await security.hashPassword('demo')
  const orders = [
    {
      orderId: _adminEmail.slice(0, 4) + '-' + utils.randomHexString(16),
    ...
    {
      orderId: demo.slice(0, 4) + '-' + utils.randomHexString(16),
      email: 'd*m*',
