...
module.exports = function login() {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query(`SELECT * FROM Users WHERE email = :email AND deletedAt IS NULL`, {
      model: UserModel, plain: true, replacements: {
        email
      },
      type: QueryTypes.SELECT
    }).then(async _user => {
      if (_user == null) return res.status(401).send(res.__('Invalid email or password.'))
      const password = await security.verifyPassword(_user?.password, req.body.password || '')
      if (!password) return res.status(401).send(res.__('Invalid email or password.'))
